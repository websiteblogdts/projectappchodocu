...
function LoginScreen({ navigation }) {
  ...
  //     console.log('Google OAuth token:', authentication.accessToken);

  //     // Gửi token này đến backend của bạn để xác thực và đăng nhập người dùng
  //   }
  // }, [response]);
  ...
  //     try {
  //       const apiURL = `${config.apiBaseURL}/user/google-login`;
  //       const googleResponse = await axios.post(apiURL, { id_token });
  //       const { token, role, user } = googleResponse.data;
  //       await storeToken(token, role, user._id);
  //       navigation.navigate(role === 'admin' ? 'Admin' : 'User');
  //     } catch (error) {
  //       console.error("Google Login error:", error);
